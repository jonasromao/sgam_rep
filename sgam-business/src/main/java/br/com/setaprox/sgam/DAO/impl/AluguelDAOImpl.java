...
public class AluguelDAOImpl extends AbstractDAO<Aluguel> implements AluguelDAO {
	...
	public List<Aluguel> reservasPorDia(Date data){
		...
		TypedQuery<Aluguel> query = em.createQuery("from Aluguel a where (a.dataInicial >= :inicio and a.dataFinal <= :fim) or (a.dataInicial >= :inicio and a.dataFinal >= :fim) order by a.dataEmissaoFaturamento desc", Aluguel.class);  
