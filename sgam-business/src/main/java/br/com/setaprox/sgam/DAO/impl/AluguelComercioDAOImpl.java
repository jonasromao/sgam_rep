...
public class AluguelComercioDAOImpl extends AbstractDAO<AluguelComercio> implements AluguelComercioDAO {
	...
	public List<AluguelComercio> reservasPorDia(Date data){
		...
		TypedQuery<AluguelComercio> query = em.createQuery("SELECT ac FROM AluguelComercio ac JOIN ac.aluguel a WHERE (a.dataInicial >= :inicio and a.dataFinal <= :fim) or (a.dataInicial <= :inicio and a.dataFinal >= :fim) or (a.dataFinal >= :inicio and a.dataFinal <= :fim) order by a.dataEmissaoFaturamento desc", AluguelComercio.class);  
